    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($role) {
                $editUrl = url('/admin/roles/' . $role->id .'/edit');
                $deleteUrl = url('/admin/roles/' . $role->id);
        ...
        <ul class="dropdown-menu">
            ...
            ->filterColumn('name', function($query, $keyword) {
                ...
                $query->where(function($q) use ($keywordLower) {
                    $q->whereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(display_name, '$.\"en\"'))) LIKE ?", ["%{$keywordLower}%"])
                      ->orWhereRaw("LOWER(JSON_UNQUOTE(JSON_EXTRACT(display_name, '$.\"ar\"'))) LIKE ?", ["%{$keywordLower}%"]);
                });
            });
    }
