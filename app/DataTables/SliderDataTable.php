    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('status', function ($q) {
                ...
                $toggleUrl = route('sliders.toggle-active', $q->id);
            ...
            ->addColumn('image', function ($slider) {
                $url = $slider->localized_image_url;
                if (!$url) $url = $slider->getFirstMediaUrl('sliders');
                
                return $url
                    ? '<img src="' . $url . '" alt="slider" style="height:50px;width:auto;object-fit:contain;border-radius:4px;">'
                    : '';
            })
            ->addColumn('action', function ($category) {
                $editUrl = url('/admin/sliders/' . $category->id.'/edit');
                $deleteUrl = url('/admin/sliders/' . $category->id);
    ...
    public function getColumns(): array
    {
        return [
            Column::make('DT_RowIndex')
                ...
                ->orderable(false)
                ->searchable(false),
            Column::make('image')->title(__('admin.image'))->addClass('text-start')->orderable(false)->searchable(false),
            Column::make('url')->title(__('admin.url'))->addClass('text-start'),
