...
@section('title', __('admin.edit') . ' . ' . $box->name)
...
@section('dashboard-main')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <h5 class="card-header">{{ __('admin.edit') . ' ' . $box->name }}</h5>
                    <div class="card-body">
                        <form id="productForm" class="row g-6" method="POST"
                                    ...
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-12">
                                                ...
                                                <input value="{{ old("name.$localeCode", $box->getTranslation('name', $localeCode)) }}"
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <textarea rows="10" class="form-control"
                                                          ...
                                                          placeholder="{{ __('admin.description') }}">{{ old("description.$localeCode", $box->getTranslation('description', $localeCode)) }}</textarea>
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <textarea class="form-control editor"
                                                          name="ingrediant_text[{{$localeCode}}]">{{ old("ingrediant_text.$localeCode", isset($box) ? $box->getTranslation('ingrediant_text',$localeCode) : '') }}</textarea>
                            ...
                            <div class="col-md-12 mb-3">
                                ...
                                <select id="category_id" class="select2 form-select" name="category_id">
                                    <option disabled value="">{{ __('admin.select_category') }}</option>
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('category_id', $box->category_id) == $category->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6 mt-3">
                                ...
                                <select class="form-select" name="type" id="productType">
                                    <option value="pickup" {{ old('type', $box->type) == 'pickup' ? 'selected' : '' }}>
                                        {{ __('admin.pickup') }}</option>
                                    <option value="delivery" {{ old('type', $box->type) == 'delivery' ? 'selected' : '' }}>{{ __('admin.delivery') }}</option>
                                    <option value="both" {{ old('type', $box->type) == 'both' ? 'selected' : '' }}>{{ __('admin.both') }}</option>
                                </select>
                            </div>

                            <div class="col-md-6 mt-3" id="branchesField">
                                ...
                                <select id="select2Multiple" class="select2 form-select" name="branches[]" multiple>
                                    @foreach($branches as $branch)
                                        <option value="{{ $branch->id }}"
                                            {{ in_array($branch->id, old('branches', $box->branches->pluck('id')->toArray())) ? 'selected' : '' }}>
                                        ...
                                        <div class="row">
                                            <div class="col-md-4 mt-3">
                                                ...
                                                <select class="form-control discount-type"
                                                    ...
                                                    <option
                                                        value="none" {{ old("prices.$currency->id.discount_type", optional($box->prices->where('currency_id', $currency->id)->first())->discount_type) == 'none' ? 'selected' : '' }}>
                                                        {{ __('admin.no_discount') }}
                                                    </option>
                                                    <option
                                                        value="fixed" {{ old("prices.$currency->id.discount_type", optional($box->prices->where('currency_id', $currency->id)->first())->discount_type) == 'fixed' ? 'selected' : '' }}>
                                                        {{ __('admin.fixed') }}
                                                    </option>
                                                    <option
                                                        value="percentage" {{ old("prices.$currency->id.discount_type", optional($box->prices->where('currency_id', $currency->id)->first())->discount_type) == 'percentage' ? 'selected' : '' }}>
                                                        {{ __('admin.percent') }}
                                                    </option>
                                                </select>
                                            ...
                                            <div class="col-md-4 mt-3">
                                                ...
                                                <div class="input-group">
                                                    <input type="number" step="0.01" min="0" class="form-control"
                                                           ...
                                                           value="{{ old("prices.$currency->id.before", optional($box->prices->where('currency_id', $currency->id)->first())->price) }}">
                                            ...
                                            <div class="col-md-4 mt-3 discount-field discount-fixed-{{ $currency->id }}"
                                                ...
                                                <div class="input-group">
                                                    <input type="number" step="0.01" min="0" class="form-control"
                                                           ...
                                                           value="{{ old("prices.$currency->id.after", optional($box->prices->where('currency_id', $currency->id)->first())->discount_price) }}">
                                                    <span class="input-group-text">{{ $currency->sign }}</span>
                                                </div>
                                            ...
                                            <div
                                                ...
                                                <div class="input-group">
                                                    <input type="number" step="1" min="0" max="100"
                                                           ...
                                                           value="{{ old("prices.$currency->id.discount_percentage", optional($box->prices->where('currency_id', $currency->id)->first())->discount_percentage) }}">
                            ...
                            @if(count($addon_groups) > 0)
                                ...
                                <div class="col-md-12">
                                    @php
                                        $selectedAddons = old('addons', $box->addons->map(function($a){ 
                                            return [
                                                'id' => $a->id,
                                                'addon_group_id' => $a->group->id ?? null,
                                                'type' => $a->pivot->type ?? 'optional',
                                                'order' => $a->pivot->order ?? 0
                                            ]; 
                                        })->toArray());
                                                ...
                                                if ($groupId) {
                                                    ...
                                                    if ($group) {
                                                        ...
                                                        foreach($addons->sortBy('order')->values() as $index => $addon) {
                                                            $addonOrderInBlock = $addon['order'] % 10000;
                                                            $addonsData[$addon['id']] = [
                                                                'id' => $addon['id'],
                                                                'type' => $addon['type'] ?? 'optional',
                                                                'order' => $addonOrderInBlock
                                                            ];
                                                        }
                                                        
                                                        $groupBlocks[] = [
                                                            'group_id' => $groupId,
                                                            'order' => floor(($addons->min('order') ?? ($blockIndex * 10000)) / 10000),
                                                            'addons' => $addonsData
                                                        ];
                                                        $blockIndex++;
                                                    }
                                                                    ...
                                                                    <div class="row mb-2 align-items-center addon-item-in-group-block" data-addon-id="{{ $addon->id }}" data-addon-group-id="{{ $addon->addon_group_id }}">
                                                                        ...
                                                                        <div class="col-md-1">
                                                                            <input type="checkbox" 
                                                                                   ...
                                                                                   {{ $isInBlock ? 'name="group_blocks[' . $blockIndex . '][addons][' . $addon->id . '][id]"' : '' }}
                                                                                   value="{{ $addon->id }}"
                                                                                   {{ $isInBlock ? 'checked' : '' }}
                            ...
                            @if(count($definitions) > 0)
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    <h6 class="text-primary m-0">{{ __('admin.nutrition') }}</h6>
                                    <button type="button" id="addDefinition" class="btn btn-secondary mt-2">{{ __('admin.add_new') }}</button>
                                </div>

                                <div class="col-md-12 mt-3">
                                    <div id="definitions-wrapper">
                                        @foreach(old('definitions', $box->properties->map(function($p){ return ['id'=>$p->id,'value'=>$p->pivot->value]; })->toArray()) as $index => $definition)
                                            <div class="row g-2 mb-2">
                                                <div class="col-md-5">
                                                    <select name="definitions[{{$index}}][id]" class="form-select definition-select">
                                                        <option value="">{{ __('admin.select') }}</option>
                                                        @foreach($definitions as $def)
                                                            <option value="{{ $def->id }}" {{ $definition['id'] == $def->id ? 'selected' : '' }}>
                                                                {{ $def->name .' - '. $def->unit }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="col-md-5">
                                                    <input type="number" step="0.1" min="0" max="100000"
                                                           name="definitions[{{$index}}][value]" class="form-control"
                                                           value="{{ $definition['value'] ?? '' }}" placeholder="Definition value">
                                                </div>
                                                <div class="col-md-2">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="active" class="select2 form-control" name="active"
                                            style="width:100%;">
                                        <option {{ old('active', $box->active ? '1' : '0') == '1' ? 'selected' : '' }} value="1">{{ __('admin.active') }}</option>
                                        <option {{ old('active', $box->active ? '1' : '0') == '0' ? 'selected' : '' }} value="0">{{ __('admin.deactive') }}</option>
                                    </select>
                                    @error('active')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="is_home" class="select2 form-control" name="is_home"
                                            style="width:100%;">
                                        <option value="0" {{ old('is_home', $box->is_home ?? false) ? '' : 'selected' }}>{{ __('admin.no') }}</option>
                                        <option value="1" {{ old('is_home', $box->is_home ?? false) ? 'selected' : '' }}>{{ __('admin.yes') }}</option>
                                    </select>
                                    @error('is_home')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" name="order" 
                                           value="{{ old('order', $box->order ?? 0) }}" min="0" 
                                           placeholder="{{ __('admin.order') }}">
                                    @error('order')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
...
@section('dashboard-footer')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const totalDefinitions = @json($definitions->count());

            function updateSelectOptions() {
                const selectedValues = Array.from(document.querySelectorAll('.definition-select'))
                    .map(select => select.value)
                    .filter(val => val !== "");

                document.querySelectorAll('.definition-select').forEach(select => {
                ...
                addBtn.disabled = wrapper.querySelectorAll('.row').length >= totalDefinitions;
            }
            ...
            <div class="col-md-5">
                <input type="number" step="0.1" min="0" max="100000"
                       name="definitions[${index}][value]" class="form-control" placeholder="Definition value">
            </div>
            <div class="col-md-2">
                <button type="button" class="btn btn-danger remove-definition">X</button>
            </div>
        `;
            ...
            wrapper.addEventListener('change', function (e) {
                if (e.target.classList.contains('definition-select')) {
                    updateSelectOptions();
                }
            });
    ...
    <script>
        'use strict';
        document.addEventListener('DOMContentLoaded', function () {
            @if(isset($box) && $box->getFirstMediaUrl('products'))
            initImageCropper('#dropzone-basic', 'image', null, "{{ $box->getFirstMediaUrl('products') }}");
            @else
    ...
    <style>
        ...
        .sortable-titles-container .sortable-ghost {
            opacity: 0.4;
            background-color: #e9ecef;
        }
        .sortable-titles-container .sortable-chosen {
            background-color: #fff3cd;
        }
        .title-card .drag-handle {
            cursor: grab;
        }
        .title-card .drag-handle:active {
            cursor: grabbing;
        }
    </style>
    ...
    <script>
        ...
        function toggleAddonInGroup(checkbox, blockIndex, addonId) {
            ...
            if (checkbox.checked) {
                // Enable the addon item
                typeSelect.disabled = false;
                orderInput.disabled = false;
                if (removeBtn) removeBtn.disabled = false;
                // Set the checkbox name so it gets submitted
                checkbox.name = `group_blocks[${blockIndex}][addons][${addonId}][id]`;
            } else {
                // Disable inputs when unchecked but keep visible
                typeSelect.disabled = true;
                orderInput.disabled = true;
                if (removeBtn) removeBtn.disabled = true;
                // Remove the checkbox name so it doesn't get submitted
                checkbox.removeAttribute('name');
            }
            // Update addon orders after toggle
            updateAddonOrdersInGroupBlock(addonItem.closest('.sortable-addons-in-group-block'));
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const wrapper = document.getElementById('subproducts-wrapper');
            const addBtn = document.getElementById('addSubProductBtn');

            addBtn.addEventListener('click', function () {
                const index = wrapper.children.length;
                const div = document.createElement('div');
                div.classList.add('row', 'g-2', 'mb-2', 'subproduct-row');
                div.innerHTML = `
            <div class="col-md-4">
                <input type="text" name="sub_products[${index}][name]" class="form-control" placeholder="{{ __('admin.name') }}">
            </div>
            <div class="col-md-4">
                <select name="sub_products[${index}][addons][]" class="form-select" multiple>
                    @foreach($all_addons as $addon)
                <option value="{{ $addon->id }}">{{ $addon->getTranslation('name', app()->getLocale()) }}</option>
                    @endforeach
                </select>
            </div>
            <div class="col-md-3">
                <input type="number" step="0.01" min="0" name="sub_products[${index}][price]" class="form-control" placeholder="{{ __('admin.price') }}">
            </div>
            <div class="col-md-1">
                <button type="button" class="btn btn-danger remove-subproduct">X</button>
            </div>
        `;
                wrapper.appendChild(div);
            });
            ...
            wrapper.addEventListener('click', function (e) {
                if (e.target.classList.contains('remove-subproduct')) {
                    e.target.closest('.subproduct-row').remove();
                }
            });
        });
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const boxTitles = @json($boxTitles ?? []);

            // Load old titles first (if validation failed), otherwise load existing titles
            const titlesToLoad = oldBoxTitles.length > 0 ? oldBoxTitles : boxTitles;
            titlesToLoad.forEach(titleData => {
                addTitle(titleData);
            });
            ...
            function addTitle(titleData = null) {
                ...
                // Convert products array to numbers for comparison
            ...
            // Before form submission, ensure all hidden inputs are synced
            const form = document.getElementById('formValidationExamples');
            if (form) {
                form.addEventListener('submit', function(e) {
                    // Sync all visible min_count inputs to their hidden counterparts
                    document.querySelectorAll('.min-count-input').forEach(function(input) {
                        const index = input.getAttribute('id').replace('min_count_', '');
                        const hiddenInput = document.getElementById(`min_count_hidden_${index}`);
                        if (hiddenInput) {
    ...
    <script>
        $(document).ready(function () {
            function toggleDiscountFields(currencyId, type) {
                const beforeField = $('.discount-before-' + currencyId + ' input');
                const fixedField = $('.discount-fixed-' + currencyId + ' input');
                const percentageField = $('.discount-percentage-' + currencyId + ' input');
                $('.discount-before-' + currencyId).hide();
                $('.discount-fixed-' + currencyId).hide();
                $('.discount-percentage-' + currencyId).hide();
                beforeField.prop('required', false).prop('disabled', true);
                fixedField.prop('required', false).prop('disabled', true);
                percentageField.prop('required', false).prop('disabled', true);
                if (type === 'fixed') {
                    $('.discount-before-' + currencyId).show();
                    $('.discount-fixed-' + currencyId).show();

                    beforeField.prop('required', true).prop('disabled', false);
                    fixedField.prop('required', true).prop('disabled', false);
                }
                if (type === 'percentage') {
                    $('.discount-before-' + currencyId).show();
                    ...
                    beforeField.prop('required', true).prop('disabled', false);
                    percentageField.prop('required', true).prop('disabled', false);
                }
            }
