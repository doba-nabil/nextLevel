...
@section('title', __('admin.edit') .' '.$branch->name)
...
@section('dashboard-main')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            ...
            <div class="col-12">
                <div class="card">
                    <h5 class="card-header">{{ __('admin.edit') .' '.$branch->name }}</h5>
                    <div class="card-body">
                        <form id="formValidationExamples" class="row g-6" method="POST"
                                    ...
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-12">
                                                ...
                                                <input value="{{ old("name.$localeCode", $branch->getTranslation('name', $localeCode)) }}"
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <textarea class="form-control" name="address[{{$localeCode}}]"
                                                          placeholder="{{ __('admin.address') }}">{{ old("address.$localeCode", $branch->getTranslation('address', $localeCode)) }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input value="{{ old('phone', $branch->phone) }}" type="text" class="form-control"
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input value="{{ old('whatsapp', $branch->whatsapp) }}" type="text" class="form-control"
                                           name="whatsapp" placeholder="{{ __('admin.whatsapp') }}">
                                    @error('whatsapp')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input value="{{ old('username', $branch->username) }}" type="text" class="form-control"
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input value="{{ old('armada_key', $branch->armada_key) }}" type="text" class="form-control"
                            ...
                            <input type="hidden" name="latitude" id="latitude" value="{{ old('latitude', $branch->lat) }}">
                            <input type="hidden" name="longitude" id="longitude" value="{{ old('longitude', $branch->lng) }}">
                            @error('latitude')
                                    ...
                                    <select id="countries" class="form-select select2" name="country_id">
                                        ...
                                        @foreach ($countries as $country)
                                            <option value="{{ $country->id }}"
                                                {{ old('country_id', $branch->location->parent->parent->id ?? '') == $country->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="states" class="form-select select2" name="states[]" multiple="multiple" style="width: 100%;">
                                        ...
                                        @php
                                            ...
                                            $savedStateIds = [];
                                            $savedStates = [];
                                            if($branch->cities->isNotEmpty()) {
                                                foreach($branch->cities as $city) {
                                                    if($city->parent && $city->parent->type == 'state') {
                                                        $stateId = $city->parent->id;
                                                        if(!in_array($stateId, $savedStateIds)) {
                                                            $savedStateIds[] = $stateId;
                                                            $savedStates[$stateId] = $city->parent->getTranslation('name', app()->getLocale());
                                                        }
                                                    }
                                                }
                                            }
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="active" class="select2 form-control" name="active"
                                            style="width:100%;">
                                        <option {{ $branch->active ? 'selected' : '' }} value="1">{{ __('admin.active') }}</option>
                                        <option {{ !$branch->active ? 'selected' : '' }} value="0">{{ __('admin.deactive') }}</option>
                                    </select>
                                    @error('active')
                                    <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                    ...
                                    <div class="working-hour-row mb-3">
                                        <div class="row g-3">
                                            <div class="col-md-3">
                                                ...
                                                <select name="working_hours[{{$index}}][from_day]" class="form-select select2">
                                                    <option value="sunday" {{ $workingHour->from_day == 'sunday' ? 'selected' : '' }}>{{ __('admin.sunday') }}</option>
                                                    <option value="monday" {{ $workingHour->from_day == 'monday' ? 'selected' : '' }}>{{ __('admin.monday') }}</option>
                                                    <option value="tuesday" {{ $workingHour->from_day == 'tuesday' ? 'selected' : '' }}>{{ __('admin.tuesday') }}</option>
                                                    <option value="wednesday" {{ $workingHour->from_day == 'wednesday' ? 'selected' : '' }}>{{ __('admin.wednesday') }}</option>
                                                    <option value="thursday" {{ $workingHour->from_day == 'thursday' ? 'selected' : '' }}>{{ __('admin.thursday') }}</option>
                                                    <option value="friday" {{ $workingHour->from_day == 'friday' ? 'selected' : '' }}>{{ __('admin.friday') }}</option>
                                                    <option value="saturday" {{ $workingHour->from_day == 'saturday' ? 'selected' : '' }}>{{ __('admin.saturday') }}</option>
                                                </select>
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">{{ __('admin.to_day') }}</label>
                                                <select name="working_hours[{{$index}}][to_day]" class="form-select select2">
                                                    <option value="sunday" {{ $workingHour->to_day == 'sunday' ? 'selected' : '' }}>{{ __('admin.sunday') }}</option>
                                                    <option value="monday" {{ $workingHour->to_day == 'monday' ? 'selected' : '' }}>{{ __('admin.monday') }}</option>
                                                    <option value="tuesday" {{ $workingHour->to_day == 'tuesday' ? 'selected' : '' }}>{{ __('admin.tuesday') }}</option>
                                                    <option value="wednesday" {{ $workingHour->to_day == 'wednesday' ? 'selected' : '' }}>{{ __('admin.wednesday') }}</option>
                                                    <option value="thursday" {{ $workingHour->to_day == 'thursday' ? 'selected' : '' }}>{{ __('admin.thursday') }}</option>
                                                    <option value="friday" {{ $workingHour->to_day == 'friday' ? 'selected' : '' }}>{{ __('admin.friday') }}</option>
                                                    <option value="saturday" {{ $workingHour->to_day == 'saturday' ? 'selected' : '' }}>{{ __('admin.saturday') }}</option>
                                                </select>
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">{{ __('admin.from_time') }}</label>
                                                <input type="time" name="working_hours[{{$index}}][from_time]" step="60"
                                                       value="{{ old('working_hours.' . $index . '.from_time', \Carbon\Carbon::parse($workingHour->from_time)->format('H:i')) }}"
                                                       class="form-control">
                                            </div>
                                            <div class="col-md-3">
                                                <label class="form-label">{{ __('admin.to_time') }}</label>
                                                <input type="time" name="working_hours[{{$index}}][to_time]" step="60"
                                                       value="{{ old('working_hours.' . $index . '.to_time', \Carbon\Carbon::parse($workingHour->to_time)->format('H:i')) }}"
                                                       class="form-control">
                                            </div>
                                            <div class="col-md-2">
                                                <button type="button" class="btn btn-danger remove-working-hour">
                                                    {{ __('admin.remove') }}
                                                </button>
                                            </div>
                                        </div>
...
@section('dashboard-footer')
    ...
    <script>
            ...
            if (categoryForm) {
                const fv = FormValidation.formValidation(categoryForm, {
                    fields: {
                        ...
                        'latitude': {
                            validators: {
                                numeric: { message: '{{ __('admin.latitude_numeric') }}' },
                                between: { min: -90, max: 90, message: '{{ __('admin.latitude_range') }}' }
                            }
                        },
                        'longitude': {
                            validators: {
                                numeric: { message: '{{ __('admin.longitude_numeric') }}' },
                        ...
                        @foreach($workingHours as $index => $workingHour)
                        'working_hours[{{$index}}][from_day]': {
                            validators: {
                                notEmpty: { message: '{{ __('admin.required') }}' }
                            }
                        },
                        'working_hours[{{$index}}][to_day]': {
                            validators: {
                                notEmpty: { message: '{{ __('admin.required') }}' }
                            }
                        },
                        'working_hours[{{$index}}][from_time]': {
                            validators: {
                                callback: {
                ...
                // إضافة تحقق ديناميكي لمواعيد العمل الجديدة
        ...
        $(document).ready(function() {
            ...
            const loadStates = debounce(function(countryId, callback) {
                ...
                // Save the currently selected state values before clearing
            ...
            $('#cities').on('change', function() {
                ...
                cityValidationTimeout = setTimeout(function() {
                    checkCityAvailability(selectedCities, {{ $branch->id }});
                }, 500);
            });
            ...
            // تحميل القيم الحالية
            @php
                $branchCities = $branch->cities->pluck('id')->toArray();
                $countryId = null;
                $stateIds = [];
                
                // Get unique states from all selected cities
                if($branch->cities->isNotEmpty()) {
                    $uniqueStates = [];
                    foreach($branch->cities as $city) {
                        if($city->parent && $city->parent->type == 'state') {
                            $stateId = $city->parent->id;
                            if(!in_array($stateId, $uniqueStates)) {
                                $uniqueStates[] = $stateId;
                                // Get country from first state
                                if($city->parent->parent && $countryId === null) {
                                    $countryId = $city->parent->parent->id;
                                }
                            }
                        }
                    }
                    $stateIds = $uniqueStates;
                }
                // If no cities, try to get from branch location
                elseif($branch->location && $branch->location->parent && $branch->location->parent->parent) {
                    $countryId = $branch->location->parent->parent->id;
                    if($branch->location->parent->type == 'state') {
                        $stateIds = [$branch->location->parent->id];
                    }
                }
                // If location exists but no parent chain, try to get country from location's parent
                elseif($branch->location && $branch->location->parent) {
                    if($branch->location->parent->type == 'state' && $branch->location->parent->parent) {
                        $countryId = $branch->location->parent->parent->id;
                        $stateIds = [$branch->location->parent->id];
                    } elseif($branch->location->parent->type == 'country') {
                        $countryId = $branch->location->parent->id;
                    }
                }
            @endphp
            ...
            $('#countries').val('{{ $countryId }}').trigger('change.select2');
            loadStates('{{ $countryId }}', function() {
                @if(!empty($stateIds))
                // Select the saved states and load cities
                const stateIdsArray = @json($stateIds);
                $('#states').val(stateIdsArray).trigger('change.select2');
                loadCities(stateIdsArray, function() {
                    @if(!empty($branchCities))
                    // Select saved cities
                    const cityIds = @json($branchCities);
                    $('#cities').val(cityIds).trigger('change.select2');
                    @endif
                });
                @endif
            });
            ...
            let hourIndex = {{ $workingHours->count() }};
            $('#add-working-hour').click(function() {
                const newRow = `
                    <div class="working-hour-row mb-3">
                        <div class="row g-3">
    ...
    <script>
        ...
        function initBranchMap() {
            ...
            const defaultLat = {{ $branch->lat ?? 29.3759 }};
            const defaultLng = {{ $branch->lng ?? 47.9774 }};
            ...
            // Ensure coordinates are set in hidden inputs on initialization
