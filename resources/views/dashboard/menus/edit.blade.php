...
@section('title', __('admin.update') .' . '. __('admin.menu'))
...
@section('dashboard-main')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <h5 class="card-header">{{ __('admin.update') .' '. __('admin.menu') }}</h5>
                    <div class="card-body">
                        <form id="formValidationExamples" class="row g-6" method="POST"
                              action="{{ route('menus.update', $menu->id) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}"
                                        ...
                                        <div class="row g-3">
                                            <div class="col-md-12">
                                                ...
                                                <input value="{{ old("name.$localeCode", $menu->getTranslation('name', $localeCode)) }}" type="text" class="form-control" name="name[{{$localeCode}}]"
                                            ...
                                            <div class="col-md-12">
                                                ...
                                                <textarea class="form-control editor"
                                                          ...
                                                          rows="5"
                                                          placeholder="{{ __('admin.content') }}">{{ old("content.$localeCode", $menu->getTranslation('content', $localeCode)) }}</textarea>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="is_active" class="select2 form-control" name="is_active"
                                            style="width:100%;">
                                        <option value="1" {{ $menu->is_active ? 'selected' : '' }}>{{ __('admin.active') }}</option>
                                        <option value="0" {{ !$menu->is_active ? 'selected' : '' }}>{{ __('admin.deactive') }}</option>
                                    </select>
                                    @error('is_active')
                            ...
                            <div class="col-12">
                                ...
                                @if($menu->getFirstMediaUrl('menus'))
                                    <div class="mt-2">
                                        <img src="{{ $menu->getFirstMediaUrl('menus') }}" alt="Current image" style="max-height: 100px;">
                                        <p class="text-muted">{{ __('admin.current_image') }}</p>
                                    </div>
                                @endif
                                @error("image")
                                <div class="invalid-feedback d-block">{{ $message }}</div>
...
@section('dashboard-footer')
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Load existing menu products - try menuProducts relationship, fallback to products with pivot
            @php
                $locale = app()->getLocale();
                $existingProductsData = [];
                if ($menu->menuProducts && $menu->menuProducts->count() > 0) {
                    $existingProductsData = $menu->menuProducts->map(function($menuProduct) use ($locale) {
                        $product = $menuProduct->product ?? null;
                        $category = $menuProduct->category ?? null;
                        $productCategory = $product ? ($product->category ?? null) : null;

                        return [
                            'id' => $menuProduct->product_id ?? ($product ? $product->id : null),
                            'category_id' => $menuProduct->category_id ?? ($product ? $product->category_id : null),
                            'name' => $product ? $product->name : '',
                            'category' => $category ? $category->getTranslation('name', $locale) : ($productCategory ? $productCategory->getTranslation('name', $locale) : ''),
                            'category_full' => $category ? $category->name : ($productCategory ? $productCategory->name : ''),
                            'show_price' => $menuProduct->show_price ?? 1
                        ];
                    });
                } else {
                    $existingProductsData = $menu->products->map(function($product) use ($locale) {
                        return [
                            'id' => $product->id,
                            'category_id' => $product->pivot->category_id ?? $product->category_id ?? null,
                            'name' => $product->name,
                            'category' => $product->category ? $product->category->getTranslation('name', $locale) : '',
                            'category_full' => $product->category ? $product->category->name : '',
                            'show_price' => $product->pivot->show_price ?? 1
                        ];
                    });
                }
            @endphp
            ...
            const currentLocale = '{{ session('admin_locale', request()->segment(1) ?: config('app.locale')) }}';
            productsData = existingProducts.map(p => {
                let productName = '';
                if (typeof p.name === 'object' && p.name !== null) {
                    if (currentLocale === 'en' || currentLocale === 'en_US' || currentLocale === 'en-GB') {
                        productName = (p.name['en'] && String(p.name['en']).trim() !== '') 
                            ? String(p.name['en']).trim() 
                            : (p.name['ar'] && String(p.name['ar']).trim() !== '' ? String(p.name['ar']).trim() : '');
                    } else {
                        productName = (p.name['ar'] && String(p.name['ar']).trim() !== '') 
                            ? String(p.name['ar']).trim() 
                            : (p.name['en'] && String(p.name['en']).trim() !== '' ? String(p.name['en']).trim() : '');
                    }
                } else {
                    productName = p.name || '';
                }
            ...
            // Load all categories (global and menu-specific for this menu)
            ...
            // Load categories for this menu initially
            loadCategories();
            updateProductsTable();
    ...
    <script>
        'use strict';
        document.addEventListener('DOMContentLoaded', function () {
            @if(isset($menu) && $menu->getFirstMediaUrl('menus'))
            initImageCropper('#dropzone-basic', 'image', null, "{{ $menu->getFirstMediaUrl('menus') }}");
            @else
