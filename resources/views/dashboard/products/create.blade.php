...
@section('dashboard-main')
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row">
            <div class="col-12">
                <div class="card">
                    <h5 class="card-header">{{ __('admin.create') . '.'. __('admin.products') }}</h5>
                    <div class="card-body">
                        <form id="productForm" class="row g-6" method="POST" action="{{ route('products.store') }}"
                                                ...
                                                <div class="col-md-5">
                                                    <select name="definitions[{{$index}}][id]"
                                                        ...
                                                        @foreach($definitions as $def)
                                                            <option
                                                                value="{{ $def->id }}"
                                                                data-key="{{ $def->key ?? '' }}">{{ $def->name .' - '. $def->unit }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="col-md-5">
                                                    <input step="0.1" min="0" max="100000"
                                                           ...
                                                           class="form-control definition-value-input"
...
@section('dashboard-footer')
    ...
    <script>
        ...
        <div class="col-md-5">
            ...
            <div class="col-md-5">
                <input type="number" step="0.1" min="0" max="100000" name="definitions[${index}][value]" class="form-control definition-value-input" placeholder="Definition value">
        </div>
        <div class="col-md-2">
    ...
    <style>
        ...
        .type-block .card-header {
            background-color: #f8f9fa;
        }
        .addon-item-in-block {
            transition: background-color 0.2s;
        }
        .addon-item-in-block:hover {
            background-color: #f8f9fa;
        }
        .sortable-type-blocks .sortable-ghost {
            opacity: 0.4;
            background-color: #e9ecef;
        }
        .sortable-type-blocks .sortable-chosen {
            background-color: #fff3cd;
        }
        .group-block {
            cursor: move;
    ...
    <script>
        ...
        function initSortableForGroup(groupId) {
            const sortableContainer = document.getElementById('sortable-group-' + groupId);
            ...
            if (sortableContainer.sortableInstance) {
                ...
                sortableContainer.sortableInstance = null;
            }
        ...
        function updateOrderForGroup(groupId) {
            const sortableContainer = document.getElementById('sortable-group-' + groupId);
            if (!sortableContainer) return;
            
            const addonItems = sortableContainer.querySelectorAll('.addon-item');
            addonItems.forEach(function(item, index) {
                const orderInput = item.querySelector('.addon-order-input');
                if (orderInput) {
                    orderInput.value = index;
                }
            });
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.addEventListener('change', function(e) {
                if (e.target.classList.contains('addon-checkbox')) {
                    ...
                    const typeSelect = addonItem.querySelector('.addon-type-select');
                    const orderInput = addonItem.querySelector('.addon-order-input');
                    ...
                    if (e.target.checked) {
                        ...
                        orderInput.value = '0';
                        addonItem.setAttribute('data-addon-type', 'optional');
                    }
                }
            });
            ...
            // Initialize sortable for all visible groups
            function initializeAllSortables() {
                document.querySelectorAll('.sortable-addons-group').forEach(function(container) {
                    const groupId = container.getAttribute('data-group-id') || container.id.replace('sortable-group-', '');
                    const groupBody = document.getElementById('group-' + groupId);
                    if (groupBody && groupBody.style.display !== 'none') {
                        initSortableForGroup(groupId);
                    }
                });
            }
            
            // Initialize immediately for visible groups
            setTimeout(initializeAllSortables, 100);
            
            // Make addon groups sortable
            const groupsContainer = document.getElementById('addon-groups-container');
            if (groupsContainer && typeof Sortable !== 'undefined') {
                groupsContainer.sortableInstance = new Sortable(groupsContainer, {
                    handle: '.card-header',
                    animation: 150,
                    ghostClass: 'sortable-ghost',
                    chosenClass: 'sortable-chosen',
                    ...
                    onEnd: function() {
                        ...
                        const groupCards = groupsContainer.querySelectorAll('.addon-group-card');
                        groupCards.forEach(function(card, index) {
                            const orderInput = card.querySelector('.group-order-input');
                            if (orderInput) {
                                orderInput.value = index;
                            }
                            card.setAttribute('data-group-order', index);
                        });
                    }
                });
            }
            
            // Re-initialize when groups are toggled
            const originalToggle = window.toggleAddonGroup;
            window.toggleAddonGroup = function(groupId) {
                if (originalToggle) originalToggle(groupId);
                setTimeout(function() {
                    const groupBody = document.getElementById('group-' + groupId);
                    if (groupBody && groupBody.style.display !== 'none') {
                        initSortableForGroup(groupId);
