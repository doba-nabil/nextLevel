                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select id="active" class="select2 form-control" name="active"
                                            style="width:100%;">
                                        <option {{ $page->active ? 'selected' : '' }} value="1">{{ __('admin.active') }}</option>
                                        @if($page->slug !== 'about-us')
                                        <option {{ !$page->active ? 'selected' : '' }} value="0">{{ __('admin.deactive') }}</option>
                                        @endif
                                    </select>
                                    @error('active')
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (pageForm) {
                const fv = FormValidation.formValidation(pageForm, {
                    fields: {
                        @foreach(LaravelLocalization::getSupportedLocales() as $localeCode => $properties)
                        "title[{{$localeCode}}]": { validators: { notEmpty: {message: '{{ $properties["name"] }} title is required'} } },
                        "content[{{$localeCode}}]": { validators: { notEmpty: {message: '{{ $properties["name"] }} content is required'} } },
                        @endforeach
                    },
                    plugins: {
